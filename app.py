...
from inference import predict_disease_from_audio
...
st.text(f"- CNN (crackles/wheezes): models/cnn_model.h5")
st.text(f"- RF enfermedad  : models/rf_3class.pkl")
...
st.markdown("---")
st.subheader("Clasificación de enfermedad (audio-only)")

try:
    # 1) extraigo el vector de 40 features
    Xg = extract_global_features(data, sr)

    # 2) saco probabilidades para cada clase
    probas = clf3.predict_proba(Xg)[0]
    labels = le3.classes_  # e.g. ['Healthy','Infectious','Obstructive']

    # 3) muestro las probabilidades
    st.write("Probabilidades:", {lab: f"{p:.3f}" for lab,p in zip(labels, probas)})

    # 4) elijo la clase con mayor probabilidad
    disease = labels[np.argmax(probas)]
    st.success(f"Predicción → **{disease}**")

except Exception as e:
    st.error(f"No pude predecir la enfermedad: {e}")
