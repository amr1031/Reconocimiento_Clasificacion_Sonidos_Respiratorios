...
# ─── 7) Clasificación de enfermedad ───
st.markdown("## Predicción de enfermedad (audio-only)")

# Llamada a inferencia
disease, probs = predict_disease_from_audio(data, sr)
st.write("Probabilidades:", probs)
st.success(f"Predicción → **{disease}**")

# Añade esto para mostrar el grupo:
group = GROUP_MAP.get(disease, "Desconocido")
st.info(f"Pertenece al grupo: **{group}**")
